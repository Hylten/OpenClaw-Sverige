...
Om du vill testa en AI-agent som är anpassad för svensk vardagspraktik kan du kontakta OpenClaw Sverige direkt via WhatsApp för ett enkelt samtal om vad du behöver.`,
  ...
  'vad-ar-en-ai-agent-excalidraw': `Att förstå vad en AI-agent är kan kännas svårt om man inte arbetar med teknik till vardags. Samtidigt börjar begreppet dyka upp överallt: i tidningar, på konferenser och i samtal på jobbet.

I den här artikeln får du en visuell förklaring med illustrationer skapade i Excalidraw.
...
### 1. Du berättar målet
Du anger vad som ska göras, inte hur det ska göras.

### 2. AI-agenten planerar
Agenten bryter ned uppdraget i delmoment: samla data, sortera, analysera, skriva, kontrollera.

### 3. Agenten genomför arbetet
Den använder AI-modellen, interna verktyg eller externa system.

### 4. Den kontrollerar sitt eget resultat
Agenten kan förbättra resultatet tills instruktionerna uppfylls.
...
*Figur 4: Till skillnad från en anställd kan AI-agenten arbeta dygnet runt.*

## Vad en AI-agent inte kan göra

Det är viktigt att ha realistiska förväntningar:

- Den tar inte ansvar för juridiska eller affärsmässiga beslut
- Den saknar sunt förnuft vid otydliga instruktioner
- Den behöver tydliga ramar för att fungera bra

## Hur OpenClaw Sverige arbetar med AI-agenter

OpenClaw Sverige erbjuder en plattform där svenska yrkesverksamma kan skapa och driva egna AI-agenter utan tekniska förkunskaper. Fokus ligger på:

- Enkelhet — Kom igång snabbt
- Kontroll — Du bestämmer vad agenten får göra
- Anpassning — Skräddarsy för din arbetsvardag

*Illustrationerna i denna artikel är skapade med Excalidraw, ett populärt ritverktyg med handdrawn-estetik.*

Om du vill testa en AI-agent kan du kontakta OpenClaw Sverige direkt via WhatsApp.`,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('### ')) {
      ...
      if (altMatch) {
        ...
        elements.push(
          <div key={key++} style={{ textAlign: 'center', margin: '32px 0' }}>
            <img 
              src={src} 
              alt={alt} 
              style={{ 
      ...
      flushList();
      elements.push(<p key={key++} style={{ fontSize: '14px', lineHeight: '1.6', color: 'rgba(26,26,26,0.5)', margin: '12px 0', fontStyle: 'italic' }}>{line.slice(1, -1)}</p>);
    } else if (line.trim() === '') {
      flushList();
    } else {
      flushList();
  ...
  <div className="page">
    <div className="site-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '48px', marginTop: '-48px', gap: '48px', flexWrap: 'nowrap', width: '100%', position: 'relative', zIndex: 100, marginBottom: '24px' }}>
     ...
     <div style={{ marginTop: '48px', marginBottom: '24px' }}>
        <Link to="/" style={{ fontSize: '13px', color: 'rgba(26,26,26,0.4)', textDecoration: 'none' }}>&larr; Tillbaka till startsidan</Link>
     </div>
...
const BlogArticle: React.FC = () => {
  ...
  const articleSlug = decodeURIComponent(encodedSlug);
  const post = allBlogPosts.find(p => p.slug === articleSlug);
  const content = allArticleContent[articleSlug];
  ...
  return (
    <div className="page sticky-header-page" style={{ paddingTop: '48px', marginTop: '-48px' }}>
      ...
      <div className="logo-area" style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', flexShrink: 0 }} onClick={() => navigate('/')}>
          <span className="logo-text">OpenClaw</span>
          <div className="logo-dot" />
          <span className="logo-tag">Sverige</span>
        </div>
      ...
      <footer className="site-footer">
        <div className="footer-content">
          <span>OpenClaw Sverige</span>
          <span className="footer-dot">·</span>
          <Link to="/blogg" className="footer-link">Blogg</Link>
          <span className="footer-dot">·</span>
          <Link to="/integritet" className="footer-link">Integritet &amp; Säkerhet</Link>
          <span className="footer-dot">·</span>
          <a href="https://hylten.github.io/Alpha/" target="_blank" rel="noopener noreferrer" className="footer-link">Roials Alpha</a>
          <span className="footer-dot">·</span>
          <a href="https://hylten.github.io/Venture-Studio/#apply" target="_blank" rel="noopener noreferrer" className="footer-link">Hyltén Venture Studio</a>
  ...
  return (
    <div className="page">
      ...
      <div style={{ marginTop: '80px', paddingTop: '48px', borderTop: '1px solid rgba(26,26,26,0.06)' }}>
        ...
        <Link to="/blogg" style={{ display: 'inline-block', marginTop: '16px', fontSize: '14px', color: 'rgba(26,26,26,0.5)', textDecoration: 'none', fontWeight: 600 }}>
          Alla artiklar →
        </Link>
      </div>
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    window.triggerTransition = (url, isBlank = false) => {
      ...
      setTimeout(() => {
        if (isBlank) { window.open(url, '_blank'); } else { navigate(url); }
  ...
  useEffect(() => {
    if (!skipIntro && !introComplete && location.pathname === '/') {
      const timer = setTimeout(() => {
        handleIntroComplete();
      }, 2800);
      return () => clearTimeout(timer);
    }
  }, [skipIntro, introComplete, location.pathname, handleIntroComplete]);
  ...
  return (
    <HelmetProvider>
      <Helmet>
        <title>OpenClaw Sverige — Din AI-agent</title>
        <meta name="description" content="OpenClaw Sverige erbjuder autonoma AI-agenter för svenska företag. Automatisera rutinarbete, förbättra kundservice och öka effektiviteten med AI." />
        <link rel="canonical" href="https://hylten.github.io/OpenClaw-Sverige/" />
      </Helmet>
      ...
      {!skipIntro && !introComplete && location.pathname === '/' && (
